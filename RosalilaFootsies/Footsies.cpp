...
//
std::pair<string, string> xreceived;

string xgetLocalPlayer1Input()
{
  ...
  if (rosalila()->receiver->isKeyDown('d') || rosalila()->receiver->isJoyDown(-6, 0))
  {
    return "6";
  }

  if (rosalila()->receiver->isKeyDown('a') || rosalila()->receiver->isJoyDown(-4, 0))
  {
    return "4";
  }
...
string xgetLocalPlayer2Input()
{
  ...
  if (rosalila()->receiver->isKeyDown('l') || rosalila()->receiver->isJoyDown(-6, 1))
  {
    return "6";
  }

  if (rosalila()->receiver->isKeyDown('j') || rosalila()->receiver->isJoyDown(-4, 1))
  {
    return "4";
  }
...
string xgetPlayer1Input(string game_type)
{
  if (game_type == "local")
  {
    return xgetLocalPlayer1Input();
  }
  if (game_type == "online")
  {
    return xreceived.first;
  }
  return "";
}

string xgetPlayer2Input(string game_type)
{
  if (game_type == "local")
  {
    return xgetLocalPlayer2Input();
  }
  if (game_type == "online")
  {
    return xreceived.second;
  }
...
Footsies::Footsies(string character1_name, string character2_name, int total_rounds, string game_type, int player)
{
  ...
  for (int i = 0; i < (int)this->character1_name.length(); ++i)
    this->character1_name[i] = toupper(this->character1_name[i]);

  this->character2_name = character2->name;
  for (int i = 0; i < (int)this->character2_name.length(); ++i)
...
void Footsies::gameLoop()
{
  while (true)
  {
    if (game_type == "online")
    {
      xreceived = rosalila()->net->receive2PlayerBuffers();
    }
    string player1_input = xgetPlayer1Input(game_type);
    string player2_input = xgetPlayer2Input(game_type);
    ...
    if (!game_started)
    {
      if (!game_over)
      {
        rosalila()->graphics->draw2DImage(counter_images[counter_animation_image],
    ...
    if (game_over)
    {
      if (player1_wins == total_rounds)
      {
        ...
        rosalila()->graphics->draw2DImage(player1_wins_images[wins_animation_image],
      ...
      else if (player2_wins == total_rounds)
      {
        ...
        rosalila()->graphics->draw2DImage(player2_wins_images[wins_animation_image],
    ...
    if (game_type == "online")
    {
      string current_player_input = xgetLocalPlayer1Input();
...
void Footsies::drawScoreboards()
{
  ...
  rosalila()->graphics->draw2DImage(scoreboard_player1,
                                    ...
                                    false,
                                    FlatShadow());
  rosalila()->graphics->draw2DImage(scoreboard_player2,
  ...
  rosalila()->graphics->draw2DImage(character1->portrait,
  ...
  rosalila()->graphics->draw2DImage(character2->portrait,
